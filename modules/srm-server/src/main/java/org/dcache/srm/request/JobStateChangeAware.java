 ...
 * JobStateChangeAware methods returns.  Therefore, a method call will not see
 ...
 * classes must not make any assumptions in which order methods are called
 * and not assume that methods are called sequentially.
 * <p>
 * Obtaining the job's read-lock from within the the call is guaranteed not to
 * block.  Therefore, the method may read any values protected only by this
 * job's read lock.
 * <p>
 * Recipients MUST NOT attempt to acquire the Job's write lock; for example, by
