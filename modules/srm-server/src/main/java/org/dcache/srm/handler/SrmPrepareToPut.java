...
public class SrmPrepareToPut
{
                   ...
                   SRMInternalErrorException
    {
        ...
        long effectiveSize = calculateEffectiveFileSize(fileRequests);
        long lifetime = Lifetimes.calculateLifetime(request.getDesiredTotalRequestTime(),
                effectiveSize, configuration.getMinimumBandwidth(),
...
/*                nextRequest.getFileStorageType()==
    ...
    /**
     * Calculate the number of bytes transferred.  We assume that multiple
     * transfers will take place in parallel, so simply return the largest file
     * size.  If the client failed to supply expected file sizes then zero is
     * returned.
     */
    private long calculateEffectiveFileSize(TPutFileRequest[] requests)
    {
