    ...
    public class ListCommand extends DelayedCommand<Serializable>
    {
        ...
        private Serializable listStatistics(Optional<ByteUnit> unitsArg)
        {
            ...
            ColumnWriter table = new ColumnWriter()
                    .headersInColumns()
                    .abbreviateBytes(!unitsArg.isPresent())
                    .header("Storage class").left("class")
                    .space().header("Total: size").bytes("totalSize", unitsArg, BINARY).header(",").fixed(" ").space().header("replicas").right("totalReplicas").header(";").fixed(" ")
                    .space().space().header("Precious: size").bytes("preciousSize", unitsArg, BINARY).header(",").fixed(" ").space().header("replicas").right("preciousReplicas").header(";").fixed(" ")
                    .space().space().header("Sticky: size").bytes("stickySize", unitsArg, BINARY).header(",").fixed(" ").space().header("replicas").right("stickyReplicas").header(";").fixed(" ")
                    .space().space().header("others: size").bytes("otherSize", unitsArg, BINARY).header(",").fixed(" ").space().header("replicas").right("otherReplicas");
            ...
            stats.entrySet().stream()
                    ...
                    .forEach(e -> {
                        ...
                        table.row()
                                ...
                                .value("test1", e.getKey())
                                .value("test2", e.getKey())
                                .value("totalSize", counter[0]).value("totalReplicas", counter[1])
                                .value("preciousSize", counter[2]).value("preciousReplicas", counter[3])
                                .value("stickySize", counter[4]).value("stickyReplicas", counter[5])
                                .value("otherSize", counter[6]).value("otherReplicas", counter[7]);
                    });
