...
public class RequestContainerV5Test
{
    ...
    public void shouldSuspendRequestWhenSuspendAll() throws Exception
    {
        ...
        var warning = thenShouldNotify(billing, WarningPnfsFileInfoMessage.class);
    ...
    public void shouldSendPoolHitInfoForSimpleReadRequest() throws Exception
    {
        ...
        var info = thenShouldNotify(billing, PoolHitInfoMessage.class);
    ...
    @Test
    public void shouldSelectHotPoolIfCostExceededWithoutTryAlternatives() throws Exception
    {
    ...
    @Test
    public void shouldStageIfReadSelectionThrowsRuntimeException() throws Exception
    {
        var stagePool = aPool("stage-pool@dCacheDomain");
        given(aPartitionManager().withDefault(aPartition()));
        given(aPoolSelectionUnit().withNetUnit("all-net", "192.168.1.1")
    ...
    @Test
    public void shouldStageIfCostExceeded() throws Exception
    {
        var stagePool = aPool("stage-pool@dCacheDomain");
        given(aPartitionManager().withDefault(aPartition()));
        given(aPoolSelectionUnit().withNetUnit("all-net", "192.168.1.1")
    ...
    public void shouldReplyMultipleRequestsWhenBelowClumpingLimit() throws Exception
    {
        ...
        List<PoolMgrSelectReadPoolMsg> allReplies = allRepliesSendWith(endpoint);
    ...
    public void shouldReturnRetryForRequestsBeyondClumpLimit() throws Exception
    {
        ...
        var allReplies = allRepliesSendWith(endpoint);
    ...
    private String given(Deliverable builder)  throws IOException,
            InterruptedException, TimeoutException, CommandException
    {
    ...
    private void whenReceiving(Deliverable builder) throws IOException, InterruptedException, TimeoutException
    {
        builder.deliverTo(container);
        waitUntilQuiescent();
    }
    ...
    private static PoolMgrSelectReadPoolMsg replySendWith(CellEndpoint endpointUsed)
    {
        var envelope = envelopeSentWith(endpointUsed);
        return (PoolMgrSelectReadPoolMsg) envelope.getMessageObject();
    }

    private static List<PoolMgrSelectReadPoolMsg> allRepliesSendWith(CellEndpoint endpointUsed)
    {
        var envelopeArg = ArgumentCaptor.forClass(CellMessage.class);
        verify(endpointUsed, Mockito.atLeastOnce()).sendMessage(envelopeArg.capture());
    ...
    private static <T extends InfoMessage> T thenShouldNotify(CellStub stub, Class<T> type)
    {
    ...
    /**
     * Deliver multiple multiple messages.  The specific messages are built
     * using an integer index, with the first message having index 1.
     */
    ...
    private class ContainerBuilder
    {
        ...
        private String stagingAuthzFile;
        ...
        public RequestContainerV5 build()
        {
            ...
            container.setStageConfigurationFile(stagingAuthzFile);
     ...
     * @param <T> the type of Message with which the cell responds.
     */
